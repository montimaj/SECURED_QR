...
<window title="Authentic QR">
  ...
  <notebook labels="Generate|Verify">
	...
	<button>
	  ...
	  <action signal="clicked">clear && java edu.sxccal.utilities.GenQR "$FILE" "$FILE_DIRECTORY"</action>
	...
	<vbox>
	  ...
	  <vbox>
	    <button>
	      ...
	      <width>40</width>
	      <label>Verify</label>
	      <action signal="clicked">clear && java edu.sxccal.utilities.Verify "$XFILE" "$XSIG" </action>
	    </button>
	  </vbox>
	 </vbox>
      </expander>
